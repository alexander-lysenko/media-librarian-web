...
import type { FormValidationRules, PasswordResetFormData, UseFormService } from '../../core/types';
...
const useDialogForm = ({ onClose }: Pick<Props, 'onClose'>): UseFormService<FormType> => {
  ...
  const usePasswordResetForm = useForm<FormType>({
    mode: 'onBlur',
  ...
  const { register, formState, reset, handleSubmit, setError, getFieldState, trigger } = usePasswordResetForm;
  ...
  return {
    ...
    dismissRootError: () => setError('root', { message: '' }),
  };
};
