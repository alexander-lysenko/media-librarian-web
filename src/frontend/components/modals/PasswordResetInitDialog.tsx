...
export const PasswordResetInitDialog = ({ open, onClose }: Props) => {
  ...
  const passwordRecoveryFormHook = usePasswordRecoveryForm(onClose);
  const { registerField, registerCaptcha, isSubmitting, errors, dismissRootError } = passwordRecoveryFormHook;
  const { handleSubmit, handleClose } = passwordRecoveryFormHook;
...
const usePasswordRecoveryForm = (onClose: Props['onClose']) => {
  ...
  return {
    ...
    isSubmitting: passwordRecoveryRequest.status === 'pending',
