...
class LibraryItemController extends ApiV1Controller
{
    ...
    {
        $itemId = $this->itemRepository->createItemGetId(libraryId: $request->libraryId, contents: $request->contents);

        $insertedItem = array_merge(['id' => $itemId], $request->contents);
        ...
        if ($request->has('posterUUID') && $request->posterUUID !== null) {
            ...
            $event = new PosterUpdated(
                ...
                oldPosterId: null,
                newPosterId: $request->posterUUID,
            );
            Event::dispatch($event);
        }

        $resource->withPoster($posterUrl ?? null);
